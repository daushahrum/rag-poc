...
3. Send messages with POST {{baseUrl}}/api/chat/send-stream using the same headers and a JSON body containing session_id and message.
4. Parse the response as Server-Sent Events. Read each data line as JSON, apply incremental response events to the UI, handle error events, and support cancellation when the screen is disposed.
5. The non-streaming fallback is POST {{baseUrl}}/api/chat/send with the same headers and body.
6. ANDI JSON responses use an envelope shaped like { "status": "success", "data": ... }. Surface useful error messages without exposing the project key.
7. Add typed request/response models, timeouts, and focused tests for session creation, streamed chunks, malformed events, HTTP errors, and cancellation.
8. Do not log the project key. Put connection values behind app configuration rather than scattering them through widgets.
...
4. Send messages with POST {{baseUrl}}/api/chat/send-stream using the same headers and a JSON body containing session_id and message.
5. Parse the response as Server-Sent Events. Read each data line as JSON, expose incremental response events to the caller, handle error events, and support AbortSignal cancellation.
6. The non-streaming fallback is POST {{baseUrl}}/api/chat/send with the same headers and body.
7. ANDI JSON responses use an envelope shaped like { "status": "success", "data": ... }. Convert non-success HTTP/envelope responses into useful application errors without leaking secrets.
8. Add timeouts and focused tests for session creation, streamed chunks, malformed events, HTTP errors, and cancellation.
